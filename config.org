...
For shell scripts, the =man-at-point= function is made, which is self-explanatory
#+begin_src emacs-lisp
...
We will also write a custom function called =gp/markdown-preview-eww=
that will allow us to compile markdown into html and render it using
=eww=. Very useful!
#+begin_src emacs-lisp
...
#+end_src
*** Prolog - Sweep
[[https://github.com/SWI-Prolog/packages-sweep][SWI-Sweep]] is a package designed for editing in *prolog*.
#+begin_src emacs-lisp
...
#+end_src
**** Org Excalidraw
[[https://github.com/wdavew/org-excalidraw?tab=readme-ov-file][org-excalidraw]] allows one to draw images utilizing [[https://excalidraw.com][excalidraw]] and have
them be inserted and displayed inline through Emacs.

Right now, this doesn't seem to be working, so we set tangle to =nil= 
...
#+end_src
** Eshell
Here we configure eshell and set up some basic configuration options
for it. Keep in mind eshell works kind of weird with certain
configuration settings so one has to call a function on the
`eshell-first-time-mode' hook in order to properly configure it.
#+begin_src emacs-lisp
...
#+end_src
** Speed Type
A fun typing game to play in Emacs
#+begin_src emacs-lisp
...
#+end_src
** Dashboard
[[https://github.com/emacs-dashboard/emacs-dashboard][emacs-dashboard]] is project that enables a nice dashboard for
Emacs. You can set it to list recent files, bookmarks, projects,
org-mode-agenda stuff, registers, and more! It also supports icons
from the =nerd-icons= package.

For now, I am using this as my dashboard, although I might want to
consider taking a page out of my Obsidian config and utilizing [[Org
Roam]]'s daily note feature.

In order to render some nice line breaks, we bring in the package
...
Here is a basic elisp function that runs a new instance of Emacs with
...
In order to easily run and manage LLMs running on the system through
Emacs, we bring in the package known as [[https://github.com/karthink/gptel][gptel]]. This package provides
functionality that allows one to interact with an LLM through a
Markdown or Org-mode buffer. It also provides one with the ability to
refactor highlighted text, code, or prose utilizing the AI. 

For this configuration, gptel is configured to work with [[https://ollama.com/][Ollama]], a
powerful LLM manager that allows one to run LLMs locally on the
system. It can also be configured to work with other LLMs, like
Chatgpt or gpt4all. More information is on the [[https://github.com/karthink/gptel][Github]]. 
...
# Local Variables:
# jinx-local-words: "Dired Docview Flyspell Keybinds Modeline Nuspell Vterm dired eshell gptel"
